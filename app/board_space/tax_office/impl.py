...
class TaxOfficeSpace(BoardSpace):
    ...
    def on_land(self, player: Player):
        ...
        # 2. 각 건물의 가치 합산
        total_value = sum(prop._building.get_price().amount for prop in player_properties)

        # 3. 세금 계산
        tax = Money(int(total_value * Decimal("0.05")))

        # 4. 소지금 확인 및 차감
        if player.get_cash() >= tax:
            player.spend(tax)
            print(f"전체 건물 가치: {total_value}원, 세금(5%): {tax}원을 납부했습니다.")
            return LandResult(f"전체 건물 가치: {total_value}원, 세금(5%): {tax}을 납부했습니다.", ["OK"], lambda _: None)
        else:
            print(f"소지금이 부족합니다! (보유 현금: {player.get_cash()}원, 세금: {tax})")
            return LandResult(f"소지금이 부족합니다! (보유 현금: {player.get_cash()}원, 세금: {tax})", ["OK"], lambda _: None)
